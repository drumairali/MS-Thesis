...
xX = [zeros(size(UsblX)) zeros(size(OdomX)) rX(1+length(rX)-length(centersUsbl):end,1)']'; # as the both centers are same
xY = [zeros(size(UsblY)) zeros(size(OdomY)) rY(1+length(rY)-length(centersUsbl):end,1)']';
[rX,yX]=getRecons(xX,W,V,yX);
[rY,yY]=getRecons(xY,W,V,yY);


decodPosX = decode(rX(1:length(rX)-length(centersUsbl),1)',[rangeUsbl rangeOdom]); # only from 
decodPosY = decode(rX(1:length(rY)-length(centersUsbl),1)',[rangeUsbl rangeOdom]); # Position


xacum=xacum+decodPosX;
yacum=yacum+decodPosY;
globalTraj=[globalTraj;t xacum yacum];
