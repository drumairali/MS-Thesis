...
if tOdom<=t && iOdom<length(odometryData) # curr
  ...
  OdomX = zeros(1,length(range));
  OdomY = zeros(1,length(range));
endif
...
if tUsbl<=t && iUsbl<length(usblData) 
  ...
  UsblX=zeros(1,length(range));
  UsblY=zeros(1,length(range));
endif
xX = [OdomX UsblX zeros(1,length(centers))]';
xY = [OdomY UsblY zeros(1,length(centers))]';
[rX,yX]=getRecons(xX,W,V,yX);
[rY,yY]=getRecons(xY,W,V,yY);

xX = [zeros(size(UsblX)) zeros(size(OdomX)) rX(1+length(rX)-length(centers):end,1)']'; # as the both centers are same
xY = [zeros(size(UsblY)) zeros(size(OdomY)) rY(1+length(rY)-length(centers):end,1)']';
[rX,yX]=getRecons(xX,W,V,yX);
[rY,yY]=getRecons(xY,W,V,yY);


decodPosX = decode(rX(1:length(rX)-length(centers),1)',[range range]); # only from 
decodPosY = decode(rX(1:length(rY)-length(centers),1)',[range range]); # Position
