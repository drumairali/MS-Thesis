...
gXDVL=gYDVL=0;
globalTrajvar=[0 0];
lastDGPS=lastUsbl=[0 0]; %reference Point
globalTraj=[];
...
#Decoding
comingVal(1,xy) = decode(r',[range range range range]);

if sum(xUSBL(xy,:))>0.1 || sum(xDGPS(xy,:))>0.1 %if there is usblData
  globalTrajvar(1,xy)+=comingVal(1,xy); %sum b/c we are taking difference
  lastUsbl(1,xy)=globalTrajvar(1,xy);    %want exact usbl difference
  lastDGPS(1,xy)=globalTrajvar(1,xy);   % for approching to dgps
  gXIMU=gYIMU=0; %reset global IMU value
  gXDVL=gYDVL=0; %reset global DVL value
  
  comingVal(1,xy) = 0; % reset comingVal as globalTrajvar is updated
endif
...
figure
plot(gtData(:,2),gtData(:,2+1))
hold on 
plot(globalTraj(:,2),globalTraj(:,2+1))
hold off
