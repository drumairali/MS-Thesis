...
pcbcrms=immse(gtData(:,2:4),globalTraj(:,2:4))
bprfrms=immse(gtData(1:6300,2:4),(allB_PR_F_Mus(2:4,:))')
...
figure(1) # PCBC-DIM
  ...
  plot3 (allPCBCDIM(2,:), allPCBCDIM(3,:), allPCBCDIM(4,:), 'b',"LineWidth", 1);
...
hold off
  ...
  err2 =  gtData(2:4,idx) - allUsbl(2:4,1:157);
  err2 = (sum((err2.*err2),1)/3).^0.5;
  hold on;
  plot(gtData(1,idx), err2(1,1:157), "b");  
  legend("B-PR-F", "USBL");
hold off
hold off
