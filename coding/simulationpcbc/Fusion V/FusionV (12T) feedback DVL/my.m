...
XlocationIMU=imuData(1,3);
XlocationDVL=dvlData(1,3);
XlocationUSBL=usblData(1,3);
...
for val=1:6300
  val
if tImu<=t && iImu<6300
  xIMU = G1D(range,imuData(iImu,3),sigmaIMU,AIMU); 
  tImu=imuData(iImu+1,1);
  iImu=iImu+1;
endif
  
if tDvl<=t && iDvl<6301 
  xDVL = G1D(range,dvlData(iDvl,3),sigmaDVL,ADVL); 
...
if tUsbl<=t && iUsbl<158
  xUSBL = G1D(range,xCusbl(iUsbl,3)-xCusbl(iUsbl-1,3),sigmaUSBL,AUSBL); 
