...
if tUsbl<=t && iUsbl<length(usblData) % as we are taking change so total-1
  xUSBL(1,:) = usblData(iUsbl,2) ;% Already a change
  xUSBL(2,:) = usblData(iUsbl,3); % Already a change
  xUSBL(3,:) = usblData(iUsbl,4);% Already a change
...
if tDGPS<=t && iDgps<length(dgpsData)
  xDGPS(1,:) = dgpsData(iDgps,2)-lastDGPS(1,1) % Already a change
  xDGPS(2,:) = dgpsData(iDgps,3)-lastDGPS(1,2)% Already a change
  xDGPS(3,:) = dgpsData(iDgps,4)-lastDGPS(1,3) % Already a change
...
comingVal(1,xyz) = (xIMU(xyz,1)+xDVL(xyz,1))/2;

if abs( xUSBL(xyz,:))>0.000001 || sum(xDGPS(xyz,:))>0.1 %if there is usblData
  globalTrajvar(1,xyz)=xUSBL(xyz,1); %sum b/c we are taking difference
...
figure(1) # PCBC-DIM
  ...
  hold on;
  title("No PC/BC-DIM Neural Network");    
  ...
  v = [-40,35];
set(gca, "linewidth", 2, "fontsize", 20)
  legend("GT", "DGPS", "USBL","No PC/BC-DIM");
  view(v);
hold off
hold off
  ...
  meanError = mean(err1);
  stdError = std(err1);
  display(strcat("No PC/BCRMS error: ", num2str(meanError)));
  display(strcat("No PC/BCstdev error: : ", num2str(stdError)));
  legend("B-PR-F", "No PC/BC-DIM");
 set(gca, "linewidth", 2, "fontsize", 20)
