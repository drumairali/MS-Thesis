...
#PCBC-DIM
xX = [xIMU xDVL xUSBL xDGPS]';
xY = [yIMU yDVL yUSBL yDGPS]';
xZ = [zIMU zDVL zUSBL zDGPS]';
...
if sum(xUSBL)>0.1 || sum(xDGPS)>0.1 %if there is usblData
  ...
  gXIMU=0; %reset global IMU value
  gXDVL=0; %reset global DVL value
...
if sum(yUSBL)>0.1 || sum(yDGPS)>0.1 %if there is usblData
  ...
  gYIMU=0; %reset global IMU value
  gYDVL=0; %reset global DVL value
...
if sum(zUSBL)>0.1 || sum(zDGPS)>0.1 %if there is usblData
  ...
  gZIMU=0; %reset global IMU value
  gZDVL=0; %reset global DVL value
